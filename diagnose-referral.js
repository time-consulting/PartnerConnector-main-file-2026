 ...
 * Run this in Replit terminal: node diagnose-referral.js
 * 
...
async function runDiagnostics() {
    try {
        ...
        if (referrers.length === 0) {
            ...
            console.log('   Run this to see all referral codes:');
            console.log('   SELECT email, referral_code FROM users WHERE referral_code IS NOT NULL;\n');
            return;
        }
        ...
        if (user7.parentPartnerId === referrer.id) {
            ...
            if (actualParent.length > 0) {
                console.log(`\n   User7 is linked to: ${actualParent[0].email}`);
